function V = TCCA_FilterBank(InImg, PatchSize, NumFilters, TCCANet, stage) 
% =======INPUT=============
...
%     c=im{3};
    tic;
    parfor i=1:N
        C_123=C_123+reshape(kron(im{3}(:,i),reshape(im{1}(:,i)*im{2}(:,i)',2401,1)),dim,dim,dim);
%         C_123=C_123+reshape(kron(c(:,i),reshape(a(:,i)*b(:,i)',2401,1)),dim,dim,dim);
    end
    clear im;
    C_123=C_123/N;
    t_C123=toc;
    ...
    for i=1:3
        im1{i}=im{i}(:,1:N/8);      
        im2{i}=im{i}(:,N/8+1:N/4);
        im3{i}=im{i}(:,N/4+1:3*N/8);
        im4{i}=im{i}(:,3*N/8+1:N/2);
        im5{i}=im{i}(:,N/2+1:5*N/8);
        im6{i}=im{i}(:,5*N/8+1:3*N/4);
        im7{i}=im{i}(:,3*N/4+1:7*N/8);
        im8{i}=im{i}(:,7*N/8+1:end);
    end
    clear im;
    tic;
    parfor i=1:n
